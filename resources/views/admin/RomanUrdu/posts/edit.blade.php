...
@php $RomanUrduPostTypeOptions = RomanUrduPostTypeOptions() @endphp

@section('title', 'Edit ' . $RomanUrduPostTypeOptions[$postType] . ' Post')
...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    <div class="flex justify-between items-center mb-1 py-2">
        <a href="{{ route('admin.roman-urdu.post.index' , ['post_type' => $postType]) }}"
    ...
    <div class="max-w-7xl">
        ...
        <form action="{{ route('admin.roman-urdu.post.update', ['postId' => $Post->id]) }}?post_type={{ $postType }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="lg:col-span-3">
                            <label class="flex items-center gap-3 px-4 py-3 bg-yellow-50 border-2 border-yellow-200 rounded-lg cursor-pointer hover:bg-yellow-100 transition">
                                <input type="checkbox"
                                    id="enable_deeplink"
                                    name="enable_deeplink"
                                    value="1"
                                    {{ old('enable_deeplink', !empty($Post->redirect_deep_link)) ? 'checked' : '' }}
                        ...
                        <div id="deeplinkFields"
                            class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:col-span-3 mt-2
                                {{ old('enable_deeplink', !empty($Post->redirect_deep_link)) ? '' : 'hidden' }}">
                            <div>
                                ...
                                <div class="flex items-center rounded-lg overflow-hidden border border-gray-300 focus-within:ring-2 focus-within:ring-[#034E7A] bg-white transition">
                                    ...
                                    <input type="text"
                                        ...
                                        value="{{ old('redirect_deep_link', $Post->redirect_deep_link) }}"
                            ...
                            <div>
                                ...
                                <select name="redirect_deeplink_post_type"
                                    ...
                                    <option value="{{ $key }}"
                                        {{ old('redirect_deeplink_post_type', $Post->redirect_deeplink_post_type) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                    @endforeach
                                </select>
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="grid  md:grid-cols-2 gap-3 border p-4 rounded max-h-[300px] overflow-y-auto">
                            @include('admin.RomanUrdu.posts.partials.edit-category-checkbox', [
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden">
                    ...
                    <textarea name="significance_content" rows="5"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#034E7A] focus:border-transparent transition">{{ old('significance_content', $Post->significance_content) }}</textarea>
                </div>
