...
@section('title', 'Edit Sahifas Shlulbayt Post')
...
<div class="max-w-6xl mt-6 bg-white p-8 rounded shadow-lg">
    ...
    <form action="{{ route('admin.english.post.update', $englishPost->id) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input name="search_text" value="{{ old('search_text', $englishPost->search_text) }}"
                class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#034E7A]">
        </div>
        ...
        <div>
            ...
            <textarea name="roman_data" rows="2" 
                class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#034E7A]">{{ old('roman_data', $englishPost->roman_data) }}</textarea>
        </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 border p-4 rounded">
                ...
                <label class="inline-flex items-center gap-2">
                    <input type="checkbox" name="category_ids[]" value="{{ $category->id }}"
                        {{ in_array($category->id, old('category_ids', $englishPost->category_ids ?? [])) ? 'checked' : '' }}>
        ...
        <div class="border-t pt-6">
            ...
            <div class="mt-4">
                ...
                <textarea name="arabic_content" rows="4" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-[#034E7A]">{{ old('arabic_content', $englishPost->arabic_content) }}</textarea>
            </div>
        </div>
        ...
        <div class="border-t pt-6">
            ...
            <div class="mt-4">
                ...
                <textarea name="transliteration_content" rows="4" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-[#034E7A]">{{ old('transliteration_content', $englishPost->transliteration_content) }}</textarea>
            </div>
            <div class="mt-4">
                <label class="block text-gray-700 mb-1">Simple Transliteration</label>
                <textarea name="simple_transliteration" rows="3" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-[#034E7A]">{{ old('simple_transliteration', $englishPost->simple_transliteration) }}</textarea>
            </div>
        </div>
        ...
        <div class="border-t pt-6">
            ...
            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea name="translation_content" rows="4" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-[#034E7A]">{{ old('translation_content', $englishPost->translation_content) }}</textarea>
                </div>
                <div>
                    <label class="block text-gray-700 mb-1">Simple Translation</label>
                    <textarea name="simple_translation" rows="3" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-[#034E7A]">{{ old('simple_translation', $englishPost->simple_translation) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="border-t pt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <input type="text" name="next_post_title" value="{{ old('next_post_title', $englishPost->next_post_title) }}" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-[#034E7A]">
            </div>
            <div>
                <label class="block text-gray-700 mb-1">Next Post URL</label>
                <input type="text" name="next_post_url" value="{{ old('next_post_url', $englishPost->next_post_url) }}" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-[#034E7A]">
            </div>
            <div>
                <label class="block text-gray-700 mb-1">Internal Link</label>
                <input type="text" name="internal_link" value="{{ old('internal_link', $englishPost->internal_link) }}" class="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-[#034E7A]">
            </div>
        </div>
    </form>
</div>
